    ...
    def process_urls(self, adapter):
        ...
        for url in adapter["urls"]:
            ...
            if not suffix:  # kein Suffix gefunden
                ...
                logging.debug(f'Found {len(matches)} newline characters in URL {cleaned_url}: {matches}')
                while not suffix and len(matches) > 0:
                    ...
                    if suffix:
                        logging.debug('Suffix found in URL after removal: %s', newline_free_url)
                        break
                    else:
                        logging.debug('No suffix found in URL after removal: %s', newline_free_url)
            ...
            if (
                    ...
                    and not newline_free_url.endswith((".pdf", ".xml", ".jpg", ".jpeg", ".png", ".md", ".txt", ".zip"))
                    and not newline_free_url.startswith("mailto")
                    and url_parts.suffix  # validate url: suffix is empty string if invalid
                    and not validators.email(newline_free_url)  # exclude email addresses not preceded by mailto scheme
            ):
