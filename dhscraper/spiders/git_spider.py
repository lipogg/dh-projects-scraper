...
class GitSpider(scrapy.Spider):
    ...
    start_urls = [
        "https://api.github.com/repos/ADHO/dh2016/contents/xml",
    ...
    def parse_abstract(self, response):
        ...
        root = (
            html.fromstring(xml_string)
            if dh2014_in_url
            else etree.fromstring(xml_string)
        )
        ...
        for parent_tag in parent_tags:
            ...
            if parent_elem is not None:
                ...
                for child_tag in child_tags:
                    ...
                    child_elems = parent_elem.findall(
                        f".//{namespace}{child_tag}"
                    )  # root.findall
                    logging.debug("Children found: %s", child_elems)
                    wf_urls = {
                        child.attrib[attr_name]
                        for child in child_elems
                        if attr_name in child.attrib
                    }
                    urls.update(wf_urls)
                    logging.debug(
                        f"Attribute matches found in {child_tag} element: {wf_urls}"
                    )
                ...
                urls.update(mf_urls)
                logging.debug(
                    f"String matches found in {parent_tag} element: {mf_urls}"
                )
